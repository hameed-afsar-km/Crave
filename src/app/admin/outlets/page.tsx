    ...
    <div className="min-h-screen bg-[#0A0A0F]">
      <div className="bg-[#0D0D14] border-b border-zinc-800/60">
        <div className="px-6 sm:px-8 py-5">
          ...
          {message && (
            <div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-medium">
        ...
        {outlets.length === 0 ? (
          ...
          <div className="grid gap-4">
            {outlets.map((outlet, i) => (
              <motion.div key={outlet.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                className="bg-[#12121A] border border-zinc-800/60 rounded-xl p-5"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-semibold text-sm text-white truncate">{outlet.name}</span>
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-xs text-zinc-500">
                      {outlet.phone && (
                        <span className="flex items-center gap-1"><Phone className="w-3 h-3" /> {outlet.phone}</span>
                      )}
                      {outlet.email && (
                        <span className="flex items-center gap-1"><Mail className="w-3 h-3" /> {outlet.email}</span>
                      )}
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {outlet.openingHours || '08:00'} - {outlet.closingHours || '23:00'}</span>
                      <span className="flex items-center gap-1"><CookingPot className="w-3 h-3" /> Prep: {outlet.preparationTime || 18}min</span>
                      <span className="flex items-center gap-1"><Users className="w-3 h-3" /> Max: {outlet.maxOrdersPerSlot || 10}/slot</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    }`} title={outlet.isOpen !== false ? 'Close' : 'Open'}>
                      {outlet.isOpen !== false ? 'Open' : 'Closed'}
                    </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowForm(false)} className="p-1 text-zinc-500 hover:text-zinc-300">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-zinc-500 mb-1">Outlet Name *</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-zinc-500 mb-1">Max Orders / Slot</label>
              ...
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.isOpen} onChange={(e) => setForm({...form, isOpen: e.target.checked})}
                    className="w-4 h-4 rounded border-zinc-600 bg-zinc-800 text-white focus:ring-zinc-500" />
                  <span className="text-xs font-medium text-zinc-300">Outlet is open</span>
                </label>
              </div>
            </div>
